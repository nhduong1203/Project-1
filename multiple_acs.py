...
class MultipleAntColonySystem:
    def __init__(self, graph: Construct_Graph, ants_num=10, beta=1, q0=0.1):
        super()
        # graph 
        self.graph = graph
        # ants_num
    ...
    """
    Hành trình di chuyển của 1 con kiến
    """
    @staticmethod
    def new_active_ant(ant: Ant, vehicle_num: int, local_search: bool, IN: np.numarray, q0: float, beta: int, stop_event: Event):
        ...
        while not ant.index_to_visit_empty() and unused_depot_count > 0:
            ...
            # Thời gian di chuyển + thời gian chờ node sẵn sàng
    ...
    @staticmethod
    def acs_time(new_graph: Construct_Graph, vehicle_num: int, ants_num: int, q0: float, beta: int,
    ...
    @staticmethod
    def acs_vehicle(new_graph: Construct_Graph, vehicle_num: int, ants_num: int, q0: float, beta: int,
                    global_path_queue: Queue, path_found_queue: Queue, stop_event: Event):
        """
        :param new_graph:
        :param vehicle_num:
